...
def help(error_no):
    ...
    print(f"    {os.path.basename(sys.argv[0])} Simple_SNP_data_file", file=sys.stderr)
    print(file=sys.stderr)
...
with open(simple_file, "r") as f:
    for line in f:
        ...
        try:
            ...
            print("The Chromosome ID should be chromosome number.\n", file=sys.stderr)
            help(1)
        ...
        print(f"rs{rs_num:08d}\t{alleles}\t{chrom}\t{pos}\t.\tNA\tNA\tNA\tNA\tNA\tNA\t" + "\t".join(genotype_data))
        rs_num += 1
